...
def get_person_center_and_facepoint(keypoints):
    """
    Return (shoulder midpoint, face point, method_used)
    Method used = 'eyes/nose' or 'shoulders'
    """
    ...
    # Try to use eyes or nose if available
    if left_eye[2] > MIN_CONFIDENCE and right_eye[2] > MIN_CONFIDENCE:
        ...
        return C_person, (face_x, face_y), "eyes/nose"
    elif nose[2] > MIN_CONFIDENCE:
        return C_person, (int(nose[0]), int(nose[1])), "eyes/nose"
    else:
        # Fallback: use shoulders (perpendicular direction to shoulder line)
        shoulder_vec = np.array([
            left_shoulder[0] - right_shoulder[0],
            left_shoulder[1] - right_shoulder[1]
        ])
        if np.linalg.norm(shoulder_vec) == 0:
            return C_person, None, None

        # Normalize
        shoulder_vec = shoulder_vec / np.linalg.norm(shoulder_vec)

        # Rotate 90 degrees to get approximate facing direction
        rotated_vec = rotate_vector_90_deg(shoulder_vec)

        # 🔁 Reverse the direction (flip 180°)
        rotated_vec = -rotated_vec

        # Estimate facing point from the person center
        face_x = int(C_person[0] + 50 * rotated_vec[0])
        face_y = int(C_person[1] + 50 * rotated_vec[1])
        return C_person, (face_x, face_y), "shoulders"
...
while cap.isOpened():
    ...
    for person_kp in keypoints_all:
        ...
        C_person, C_face, method = get_person_center_and_facepoint(kps)
        ...
        if np.linalg.norm(V_facing) != 0:
            ...
            arrow_color = (0, 255, 255) if method == "eyes/nose" else (0, 100, 255)
            cv2.arrowedLine(frame, C_face, arrow_tip, arrow_color, 3, tipLength=0.3)
            cv2.putText(frame, f"Facing ({method})", (C_person[0]-60, C_person[1]-70),
    ...
    if cv2.waitKey(1) & 0xFF == 27:
        break
    # Store frames as video: create writer once and write each frame
    if 'out' not in globals() or out is None:
        out = cv2.VideoWriter('output_video_facing.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20.0, (frame.shape[1], frame.shape[0]))
    out.write(frame)

# After loop ends, release writer if it was created
